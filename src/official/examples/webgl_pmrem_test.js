...
const exampleInfo = {
  ...
  info: [
    [
      ...
      {
        tag: 'text',
        content: '- PMREM test by'
      },
      {
        tag: 'a',
        link: 'https://github.com/elalish',
        content: 'Emmett Lalish'
      }
    ],
    [
      {
        tag: 'text',
        content: '1: white metal. 2: white dielectric. 3: black dielectric.'
      }
    ],
    [
      {
        tag: 'text',
        content: 'PMREM on: HDR with a single bright pixel. PMREM off: DirectionalLight.'
      }
    ],
    [
      {
  ...
  init: ({ window, canvas, GUI, Stats, needToDispose, useFrame }) => {
    ...
    function updateCamera() {
      ...
      const verticalFoV =
        (2 * Math.atan(Math.tan(((horizontalFoV / 2) * Math.PI) / 180) / camera.aspect) * 180) /
        Math.PI;
